...
def run(configfile):
    ...
    try:
        ...
        for idx, line in enumerate(dataloader.get_data()):
            ...
            # Process line
            eq_coordinates = coordinates.Equatorial(ra=line[0], dec=line[1], distance=line[2], mualpha=line[3], mudelta=line[4], radialvelocity=line[5], unit=('deg', 'deg', 'pc', 'mas/a', 'mas/a', 'km/s'))
            gal_coordinates = eq_coordinates.transform_to('galactic')
            current_row = [
                           gal_coordinates.get_basic('x', 'pc'),
                           gal_coordinates.get_basic('y', 'pc'),
                           gal_coordinates.get_basic('z', 'pc'),
                           gal_coordinates.get_basic('xdot', 'km/s'),
                           gal_coordinates.get_basic('ydot', 'km/s'),
                           gal_coordinates.get_basic('zdot', 'km/s')] + line[6:]
